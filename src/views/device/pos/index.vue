  ...
  <div class="device-pos">
    ...
    <Modal v-model:open="detailVisible" title="设备详情" width="600px" :footer="null">
      <Descriptions :column="2" bordered v-if="currentDevice">
        ...
        <DescriptionsItem label="设备类型">POS机</DescriptionsItem>
...
async function fetchData() {
  loading.value = true;
  try {
    const params: any = { page: pagination.current, pageSize: pagination.pageSize, deviceType: 4 };
    if (searchForm.deviceNo) params.deviceNo = searchForm.deviceNo;
    if (searchForm.mchNo) params.mchNo = searchForm.mchNo;
    if (searchForm.status !== undefined) params.status = searchForm.status;

    const res = await defHttp.get({ url: '/basic-api/device/pos/list', params });
...
async function handleAdd() {
  ...
  try {
    await defHttp.post({ url: '/basic-api/device/register', params: { ...addForm, deviceType: 4 } });
