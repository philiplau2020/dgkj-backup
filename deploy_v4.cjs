    ...
    return new Promise((resolve, reject) => {
        conn.on('ready', async () => {
            try {
                // 上传
                console.log('[1/4] 上传...');
                await uploadFile(conn, 'D:/DGKJ/server_build4.zip', '/tmp/dgkj_server4.zip');
                ...
                await execCommand(conn, 'cd /opt/dgkj-server && unzip -o /tmp/dgkj_server4.zip');
                await execCommand(conn, 'cd /opt/dgkj-server && pm2 start ecosystem.config.js');

                await new Promise(r => setTimeout(r, 3000));

                // 全面测试
                console.log('[4/4] 全面测试...');
                ...
                const apis = [
                    ...
                    ['/basic-api/channel/channel/list', '通道列表(重复)'],
                    ['/basic-api/channel/mch/list', '通道商户列表'],
