...
async function check() {
    ...
    return new Promise((resolve, reject) => {
        conn.on('ready', async () => {
            ...
            await execCommand(conn, `curl -s "http://localhost:3000/basic-api/sys/config/list" -H "Authorization: Bearer ${token}" > /dev/null`);

            await new Promise(r => setTimeout(r, 1000));

            // 查看错误
            console.log('--- 最新错误 (前30行) ---');
            const logs = await execCommand(conn, 'pm2 logs dgkj-server --err --lines 30 --nostream 2>&1 | grep -A 20 "QueryFailedError"');
            console.log(logs);
