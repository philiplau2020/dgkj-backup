...
async function deploy() {
    ...
    return new Promise((resolve, reject) => {
        conn.on('ready', async () => {
            try {
                console.log('[1/5] 上传...');
                await uploadFile(conn, 'D:/DGKJ/server_build7.zip', '/tmp/dgkj_server7.zip');
                ...
                await execCommand(conn, 'cd /opt/dgkj-server && unzip -o /tmp/dgkj_server7.zip');
                ...
                const tests = [
                    ...
                    ['/basic-api/mch/app/list', '商户应用列表'],
                    ['/basic-api/mch/store/list', '商户门店列表'],
                ];
