...
async function deploy() {
    ...
    try {
        ...
        await ssh.putFiles([
            { local: distZip, remote: '/tmp/dgkj_deploy.zip' }
        ]);
        console.log('上传完成\n');
        ...
        // 验证
        console.log('验证 API...');
        const verifyResult = await ssh.execCommand('curl -s http://localhost:3000/basic-api/sys/config/list');
        console.log('API 响应:', verifyResult ? verifyResult.substring(0, 200) : '成功');
